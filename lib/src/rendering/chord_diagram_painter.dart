...
class ChordDiagramPainter extends CustomPainter {
  ...
  ChordDiagramPainter({
    ...
    this.stringSpacing = 24.0,
    this.fretSpacing = 28.0,
  });

  @override
  void paint(Canvas canvas, Size size) {
    final effectiveMarkers = chord.effectiveStringMarkers;
    ...
    if (chord.startFret == 1) {
      final Paint nutPaint = Paint()
        ...
        ..strokeWidth = 4.5;
      canvas.drawLine(
        Offset(gridLeft - 1, gridTop),
        Offset(gridRight + 1, gridTop),
        nutPaint,
      );
    } else {
      ...
      fretLabelPainter.paint(
        canvas,
        Offset(
          gridLeft - fretLabelPainter.width - 8,
          gridTop + (fretSpacing / 2) - fretLabelPainter.height / 2,
        ),
      );
    }
    ...
    titlePainter.paint(
      canvas,
      Offset((gridLeft + gridRight) / 2 - titlePainter.width / 2, 4.0),
    );
    ...
    for (int i = 0; i < 6; i++) {
      ...
      if (markerIndex < 0 || markerIndex >= effectiveMarkers.length) continue;

      final marker = effectiveMarkers[markerIndex];
      ...
      if (marker == ChordStringMarker.muted) {
        ...
        xPainter.paint(
          canvas,
          Offset(markerX - xPainter.width / 2, markerY - xPainter.height / 2),
        );
        ...
        oPainter.paint(
          canvas,
          Offset(markerX - oPainter.width / 2, markerY - oPainter.height / 2),
        );
      }
    }
    ...
    for (final placement in chord.placements) {
      if (placement.barreEndString != null) {
        ...
        final double fretY =
            gridTop + relativeFret * fretSpacing + (fretSpacing / 2);
        ...
        final double xEnd =
            gridLeft + (6 - placement.barreEndString!) * stringSpacing;
    ...
    for (final placement in chord.placements) {
      ...
      if (placement.finger != null) {
        ...
        fingerPainter.paint(
          canvas,
          Offset(
            dotX - fingerPainter.width / 2,
            dotY - fingerPainter.height / 2,
          ),
        );
      }
    }
  ...
  bool shouldRepaint(covariant ChordDiagramPainter oldDelegate) {
    return oldDelegate.chord != chord ||
        ...
        oldDelegate.fretSpacing != fretSpacing;
  }
}
