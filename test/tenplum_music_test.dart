...
void main() {
  group('tenplum_music Package Tests', () {
    ...
    test('Note models correctly distinguish rests, pitches, and tablatures', () {
      const note = Note(pitch: Pitch.d4, duration: NoteDuration.quarter, startBeat: 0.0);
      const tabNote = Note(string: 6, fret: 3, duration: NoteDuration.quarter, startBeat: 0.0);
    ...
    testWidgets('SheetMusicView renders without errors', (WidgetTester tester) async {
      final measures = [
        const Measure(
          number: 1,
          notes: [
            Note(pitch: Pitch.d4, duration: NoteDuration.quarter, startBeat: 0.0),
            Note(pitch: Pitch.e4, duration: NoteDuration.quarter, startBeat: 1.0),
    ...
    testWidgets('TablatureView renders without errors', (WidgetTester tester) async {
      final measures = [
        const Measure(
          number: 1,
          notes: [
            Note(string: 6, fret: 10, duration: NoteDuration.quarter, startBeat: 0.0),
            Note(string: 5, fret: 12, duration: NoteDuration.quarter, startBeat: 1.0),
            Note(string: 4, fret: 12, duration: NoteDuration.quarter, startBeat: 2.0),
          ],
        ),
      ];
    ...
    testWidgets('ChordDiagramView renders on Canvas without errors', (WidgetTester tester) async {
      const chord = ChordDefinition(
        name: 'Dm7',
        startFret: 1,
        placements: [
    ...
    testWidgets('CustomFretboard resolves taps and triggers onNoteTapped callback', (WidgetTester tester) async {
      String? tappedNote;
      int? tappedString;
      int? tappedFret;
    ...
    testWidgets('CustomFretboard renders with custom FretboardStyle configuration', (WidgetTester tester) async {
      await tester.pumpWidget(
        const MaterialApp(
          home: Scaffold(
            body: CustomFretboard(
